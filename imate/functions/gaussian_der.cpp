...
GaussianDer::GaussianDer(double mu_, double sigma_)
{
    this->mu = mu_;
    this->sigma = sigma_;
}
...
float GaussianDer::function(const float lambda_) const
{
    ...
    return -(x * 0.5f * M_SQRT1_2 * M_2_SQRTPI / sigma2) * \
        std::exp(-0.5 * x * x);
}
...
double GaussianDer::function(const double lambda_) const
{
    ...
    return -(x * 0.5 * M_SQRT1_2 * M_2_SQRTPI / sigma2) * \
        std::exp(-0.5 * x * x);
}
...
long double GaussianDer::function(const long double lambda_) const
{
    ...
    return -(x * 0.5l * M_SQRT1_2 * M_2_SQRTPI / sigma2) * \
        std::exp(-0.5 * x * x);
}
