...
def toeplitz_trace(a, b, size, gram=False, p=1.0):
    """
    ...
    For the matrix :math:`\\mathbf{A}` given in :func:`imate.toeplitz`, this
    function computes :math:`\\mathrm{trace}(\\mathbf{A}^p)` where :math:`p` is
    a given exponent.
    ...
    p : float, default=1.0
        The exponent :math:`p` in :math:`\\mathbf{A}^p`. If ``gram`` is `True`,
        then :math:`p` can only be `1.0`.
    ...
    if gram:
        ...
        if p != 1.0:
            raise ValueError('When "gram" is "True", "p" should be "1".')
...
def toeplitz_traceinv(a, b, size, gram=False, p=1.0):
    """
    ...
    For the matrix :math:`\\mathbf{A}` given in :func:`imate.toeplitz`, this
    function computes :math:`\\mathrm{trace}(\\mathbf{A}^{-p})` where :math:`p`
    is a given exponent.
    ...
    .. math::
        ...
        \\mathrm{trace}(\\mathbf{A}^{-1}) = \\frac{n}{a^p},

    where :math:`n` is the size of the matrix. For the Gramian matrix,
    :math:`\\mathbf{B} = \\mathbf{A}^{\\intercal} \\mathbf{A}` (when ``gram``
    is set to `True`), and assuming :math:`p=1`, the trace of inverse is
...
def toeplitz_logdet(a, b, size, gram=False, p=1.0):
    """
    ...
    For the matrix :math:`\\mathbf{A}` given in :func:`imate.toeplitz`, this
    function computes :math:`\\mathrm{logdet}(\\mathbf{A}^p)` where :math:`p`
    is a given exponent.
    ...
    .. math::
        ...
        \\mathrm{logdet}(\\mathbf{A}) = n p \\log_e(a),

    where :math:`n` is the size of the matrix. For the Gramian matrix,
    :math:`\\mathbf{B} = \\mathbf{A}^{\\intercal} \\mathbf{A}` (when ``gram``
    is set to `True`), and assuming :math:`p=1`, the log-determinant is
