...
def GenerateBasisFunctions(n,m):
    ...
    The design matrix :math:`\mathbf{X}` here is generated so that it is nearly singular. 
    Using singular value decomposition, we generate :math:`\mathbf{X}` by

    .. math::

        \mathbf{X} = \mathbf{U} \mathbf{\Simga} \mathbf{V}^{\intercal},

    where :math:`\mathbf{U}` and :math:`\mathbf{V}` are orthonormal matrices and :math:`\mathbf{\Sigma}` is
    thr diagonal matrix of singular values.

    The orthonormal matrices are generated by Householer reflection

    .. math::

        \mathbf{U} = \mathbf{I} - 2 \\frac{ \\boldsymbol{u} \otimes \\boldsymbol{u}}{\| \\boldsymbol{u} \|^2}

    and 

    .. math::

        \mathbf{V} = \mathbf{I} - 2 \\frac{ \\boldsymbol{v} \otimes \\boldsymbol{v}}{\| \\boldsymbol{v} \|^2}

    where :math:`\\boldsymbol{u}` and :math:`\\boldsymbol{v}` are random vectors. Also, the singular values 
    (diagonals of :math:`\mathbf{\Sigma}`) are produced by

    .. math::

        \sigma_i = \exp \\left( -40 \\left( \\frac{i}{m} \\right)^{\\frac{3}{4}} \\right).

    The reason to choose such function for singular values is that the generalized cross-validation function 
    :math:`V(\\theta)`, which later we will plot, will have two non-trivial minima. The two minima is
    interesting to study for our application.
    """
...
def GenerateNoisyData(X,NoiseLevel=4e-1):
    ...
    where :math:`\mathbf{X}` is the design matrix and :math:`\\beta` is the coefficients of the 
    linear function.

    :param X: Design matrix
    :type X: numpy.ndarray

    :param NoiseLevel: Standard deviation of additive Gaussian noise.
    :type NoiseLevel: float
    """
...
def GenerateMatrix(n=1000,m=500,Shift=1e-3):
    ...
    where :math:`\mathbf{X}` is of the size :math:`n \\times m`. Since :math:`\mathbf{K}_0` 
    is singular, we add a slight shift to its diagonals
    ...
    :param Shift: A small number to be added to the diagonal elements of the matrix
        (in case the generated matrix is singular) to make the matrix slightly non-singular.
    :type Shift: float
    """
