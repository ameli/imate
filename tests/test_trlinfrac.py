...
# ======================
# test trlinfrac methods
# ======================

def _test_trlinfrac_methods(K, matrix, gram, p, coeff, assume_matrix):
    """
    Computes the trlinfrac of matrix ``K`` with multiple method.
    ...
    time10 = time.time()
    trlinfrac1 = trlinfrac(K, gram=gram, p=p, coeff=coeff, method='eigenvalue',
    ...
    time20 = time.time()
    trlinfrac2 = trlinfrac(K, gram=gram, p=p, coeff=coeff, method='slq',
    ...
    # Exact solution of trlinfrac for band matrix
    trlinfrac_exact = trlinfrac1

    # error
    error1 = relative_error(trlinfrac1, trlinfrac_exact)
    error2 = relative_error(trlinfrac2, trlinfrac_exact)
    ...
    print('eigenvalue  N/A                       %8.3f  %5.2f%%  %5.2f'
          % (trlinfrac1, error1, elapsed_time1))
    print('slq         N/A                       %8.3f  %5.2f%%  %5.2f'
          % (trlinfrac2, error2, elapsed_time2))
    print('-------------------------------------------------------------')
    print('')
...
def test_trlinfrac():
    """
    A test for :mod:`imate.trlinfrac` sub-package.
    """
    ...
    for dtype in dtypes:
        for gram in grams:
            ...
            #     2. We compute trlinfrac of K.T @ K using only K (hence we set
            #        gram=True in trlinfrac method).
            #
            # When gram is False:
            #     1. We generate a 3-band symmetric matrix K (hence we set
            #        gram=True in toeplitz).
            #     2. We compute trlinfrac of K using K (hence we set
            #        gram=False in trlinfrac method).
            ...
            for sparse in sparses:
                ...
                for p in exponents:
                    ...
                    _test_trlinfrac_methods(K, matrix, gram, p, coeff,
                                            assume_matrix)
